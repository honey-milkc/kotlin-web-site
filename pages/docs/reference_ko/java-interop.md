...
왜냐면 코틀린은 현재 쓰기 전용 프로퍼티를 지원하지 않기 때문이다.

## void를 리턴하는 메서드

void를 리턴하는 자바 메서드를 코틀린에서 호출하면 `Unit`을 리턴한다.
혹시 리턴 값을 사용하면, 값 자체를 미리 알 수 있기 때문에(`Unit`) 코틀린 컴파일러는 호출 위치에 값을 할당한다.  
...
코틀린에서는 *플랫폼 타입*이라고 불리는 자바 선언 타입으로 별도로 처리한다.
...
대신 널포인터 익셉션이나 코틀린이 null이 전파되는 것을 막기 위해 생성한 단언때문에 런타임에 호출이 실패할 수 있다. 
...
플랫폼 값을 코틀린 변수에 할당하면, 타입 추론에 의존하거나(위 예에서 `item`은 추로한 플랫폼 타입을 가짐)
또는 기대하는 타입을 선택할 수 있다(nullable과 non-null 타입 둘 다 허용).
...
non-null 타입을 선택하면 컴파일러는 할당 위치에 단언을 발생(emit)할 것이다. 이는 코틀린의 non-null 변수가 null 값는 것을 막아준다.
단언은 또한 non-null 값을 기대하는 코틀린 함수에 플랫폼 값을 전달할 때에도 발생한다.
전반적으로 컴파일러는 null이 프로그램에 퍼지는 것을 막기 위해 최선을 다한다(지네릭 때문에 완전히 없애는 것은 불가능하다). 

### 플랫폼 타입을 위한 표기

위에서 언급한 것처럼, 플랫폼 타입을 프로그램에서 직접 언급할 수 없어서 언어에 플랫폼 타입을 위한 구문이 없다.
그럼에도 불구하고 컴파일러와 IDE는 때때로 플랫폼 타입을 표시해야 할 필요성이 있으며(에러 메시지에서나 파라미터 정보 등),
따라서 이를 위한 기억용 표기를 제공한다:

* `T!`은 "`T` 또는 `T?`"를 의미한다.
* `(Mutable)Collection<T>!`은 "`T`의 자바 콜렉션이 수정가능하거나 불변일 수 있고, nullable이거나 non-null일 수 있다"는 것을 의미한다.
* `Array<(out) T>!`은 "`T`의(또는 `T`의 하위타입의) 자바 배열이 nullable이거나 아닐 수 있다"를 의미한다.

### 널가능성 애노테이션

널가능성(nullability) 애노테이션을 가진 자바 타입은 플랫폼 타입이 아닌 실제 nullable이나 not-null 코틀린 타입으로 표현한다.
...
`@Nonnull(when = ...)` 값이 `When.ALWAYS`이면, 애노테이션 붙은 타입을 non-null로 다룬다. `When.MAYBE`와 `When.NEVER`는
nullable 타입을 표시한다. `When.UNKNOWN`은 타입을 [플랫폼 타입](#null-safety-and-platform-types)으로 강제한다.
...
애노테이션 타입에 [`@TypeQualifierNickname`](https://aalmiray.github.io/jsr-305/apidocs/javax/annotation/meta/TypeQualifierNickname.html)와
JSR-305 `@Nonnull`(또는 이것의 다른 이름인 `@CheckForNull`와 같은 것)을 함께 붙이면,
정확한 널가능성을 검색하는데 그 애노테이션 타입 자체를 사용할 수 있고, 적용한 널가능성 애노테이션과 동일한 의미를 갖는다:
...
[`@TypeQualifierDefault`](https://aalmiray.github.io/jsr-305/apidocs/javax/annotation/meta/TypeQualifierDefault.html)은
애노테이션을 도입하는데, 그 애노테이션을 적용할 때 애노테이션을 붙인 요소의 범위 안에서 기본 널가능성을 정의한다.  
...
타입 용도와 일치하는 `ElementType`을 가진 타입 한정자 디폴트 애노테이션을 붙인 가장 안쪽을 둘러싼 요소가 기본 널가능성을 결정한다.
...
코틀린 1.1.50+/1.2 버전의 기본 동작은 `-Xjsr305=warn`와 같다. `strict` 값은 실험적으로만 고려해야 한다(향후에 더 다양한 검사를
추가할 것이다).

## 매핑된 타입

코틀린은 일부 자바 타입을 특별하게 다룬다. 그런 타입은 자바의 "as is"로 로딩하지 않고 대응하는 코틀린 타입으로 _매핑_한다.
...
자바의 박싱된 기본 타입은 nullable 코틀린 타입으로 매핑한다:
...
콭렉션 타입은 코틀린에서 읽기 전용이나 변경가능이 존재하므로 자바 콜렉션을 다음과 같이 변경한다(이 표에서 모든 코틀린 타입은 
...
자바와 같이 코틀린 지네릭은 런타임에 유지되지 않는다. 예를 들어, 객체는 생성자에 전달된 실제 타입 인자에 대한 정보를 담지 않으므로
...
이런 경우를 다루기 위해 기본 모든 기본 타입 배열을 위한 특수 클래스((`IntArray`, `DoubleArray`, `CharArray` 등)를 제공하고 있다.
...
인덱스를 갖고 다룰 때 조차도 오버헤드가 발생하지 않는다:
...
이 경우 `IntArray`에 `*` 연산자(spread 연산자)를 사용해서 배열을 전달할 수 있다:
...
코틀린에서 모든 익셉션은 unchecked다. 이는 컴파일러가 익셉션의 처리(catch)를 강제하지 않음을 의미한다.
따라서 checked 익셉션을 선언한 자바 메서드를 호출할 때 콭르린은 어떤 것도 강제하지 않는다.
...
위 코드는 코틀린 1.1부터 지원하는 [묶인 클래스 레퍼런스](reflection.html#bound-class-references-since-11)
`javaClass` 확장 프로퍼티를 사용할 수도 있다:
...
`java.lang.Class`를 통해 자바 리플렉션을 사용
`instance::class.java`, `ClassName::class.java`, `instance.javaClass`를 사용해서 자바 리플렉션에 필요한
`java.lang.Class`를 구할 수 있다.

자바 getter/setter 메서드 또는 코틀린 프로퍼티를 위한 backing 필드 구하기, 자바 필드를 위한 `KProperty` 구하기,
