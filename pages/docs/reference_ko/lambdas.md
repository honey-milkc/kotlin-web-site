...
`lock`으로 보호하는 동안 *try*{: .keyword } 블록에서 이 함수를 호출하고 그 결과를 `lock()` 함수에 의해 리턴한다.
...
코틀린에서 함수의 마지막 파라미터가 함수면,
괄호 밖에서 람다 식을 인자로 전달할 수 있다.
...
함수를 호출할 때 인자에 람다밖에 없으면 괄호를 완전히 생략할 수 있음에 주목하자.
...
람다 식 또는 임의 함수는 함수 선언 없이 바로 식으로 전달한 함수인 "함수 리터럴"이다. 다음 예를 보자:

``` kotlin
max(strings, { a, b -> a.length < b.length })
```

`max` 함수는 고차 함수로 두 번째 인자로 함수 값을 취한다.
두 번째 인자는 자체가 함수인 식으로 함수 리터럴이다. 함수로서 다음과 동등하다:
...
함수 타입을 nullable 변수를 선언하려면 전체 함수 타입을 괄호로 감싸고 그 뒤에 물음표룔 붙인다:
...
`it`이라는 이름으로 파라미터를 implicit하게(자동으로? 암묵적으로?) 선언한다: 
It's very common that a lambda expression has only one parameter.
If Kotlin can figure the signature out itself, 
it allows us not to declare the only parameter, and will implicitly
...
[한정한 리털](returns.html#return-at-labels) 구문을 사용해서 람다에서 값을 명시적으로 리턴할 수 있다.
...
See the grammar for [callSuffix](grammar.html#callSuffix)에 대한 문법을 참고한다.

### 임의 함수

위에서 보여준 람다 식 구문에서 빼 먹은 한 가지는 함수의 리턴 타입을 지정할 수 있다는 점이다.
많은 경우, 리턴 타입을 자동으로 유추할 수 있기 때문에 이것이 불필요하다.
하지만, 명확하게 리턴 타입을 지정하고 싶다면 다른 구문은 _임의 함수_를 사용할 수 있다.

``` kotlin
fun(x: Int, y: Int): Int = x + y
```

임의 함수는 일반 함수 선언과 비슷해 보인다. 차이점은 이름을 생략했다는 점이다.
...
임의 함수에 대한 리턴 타입 추론은 보통 함수와 동일하게 동작한다.
몸체가 식인 임의 함수에 대한 리턴 타입은 자동으로 유추한다.
블록 몸체를 가진 임의 함수의 경우 명시적으로 리턴 타입을 지정해야 한다(아니면 `Unit`으로 가정).

임의 함수 파라미터는 항상 괄호 안에 전달해야 한다.
괄호 밖에 함수 전달을 허용하는 간단 구문은 람다 식에 대해서만 동작한다.

람다 식과 임의 함수 간의 한 가지 다른 차이점은 [비로컬 리턴](inline-functions.html#non-local-returns)의 동작 방식이다.
라벨이 없는 *return*{: .keyword } 문장은 항상 *fun*{: .keyword } 키워드로 선언한 함수에서 리턴한다.
이는 람다 식의 *return*{: .keyword }은 둘러싼 함수를 리턴하는 반면
임의 함수의 *return*{: .keyword }은 임의 함수 자체로부터 리턴한다는 것을 의미한다.

### 클로저

람다 식 또는 임의 함수(그리고 [로컬 함수](functions.html#local-functions)와 [오브젝트 식](object-declarations.html#object-expressions))은  
...
임의 함수 구문은 함수 리터럴의 리시버 타입을 직접 지정할 수 있게 한다.
...
문맥에서 리시버 타입을 추론할 수 있는 경우
람다 식을 리시버를 가진 함수 리터럴로 사용할 수 있다.
