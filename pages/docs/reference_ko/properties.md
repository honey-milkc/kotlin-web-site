...
`lateinit`는 (주요 생성자에 포함되지 않고 프로퍼티가 커스텀 게터나 세터를 갖지 않을 때만) 클래스 몸체에 선언한 `var` 프로퍼티에 사용할 수 있고,
코틀린 1.2부터는 최상위 프로퍼티와 로컬 변수에 사용할 수 있다.
프로퍼티나 변수의 타입은 not-null이어야 하고 기본 타입이면 안 된다.

`lateinit` 프로퍼티를 초기화하기 전에 접근하면 특수한 익셉션을 발생한다.
이 익셉션은 접근한 프로퍼티와 아직 초기화하지 않았다는 것을 명확하게 식별한다.

### lateinit var 초기화 여부 검사 (1.2부터)

`lateinit var`를 이미 초기화했는지 확인하려면 
[그 프로퍼티에 대한 참조](reflection.html#property-references)에 `.isInitialized`를 사용하면 된다:
...
이 검사는 의미적으로 접근 가능한 프로퍼티에 대해서만 가능하다. 예를 들어, 같은 타입에 선언되었거나 
외부 타입에 선언되었거나 또는 같은 파일의 최상위에 선언된 프로퍼티에 대해 가능하다. 
