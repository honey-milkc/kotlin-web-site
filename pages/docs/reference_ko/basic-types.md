...
* 십진수: `123`
  * Long은 대문자로 `L`로 표시: `123L`
* 16진수: `0x0F`
* 2진수: `0b00001011`

주의: 8진수 리터럴은 지원하지 않음

코틀린은 또한 부동소수점을 위한 표기법을 지원한다:
...
만약 nullable 숫자 레퍼런스(예 `Int?`)가 필요하거나 지네릭이 관여하면 박싱 타입으로 저장한다. 
...
하위 타입이 된다 다음과 같은 문제가 발생한다:

``` kotlin
// 가상의 코드로, 실제로는 컴파일되지 않음:
val a: Int? = 1 // 박스된 Int (java.lang.Integer)
val b: Long? = a // 자동 변환은 박스된 Long을 생성 (java.lang.Long)
print(a == b) // 놀랍게도! 이는 "false"를 리턴한다. 왜냐면 Long의 equals()은 비교 대상도 Long인지 검사하기 때문이다
```

동일성뿐만 아니라 동등함조차 모든 곳에서 나도 모르게 사라지게 된다.

따라서 작은 타입을 큰 타입으로 자동으로 변환하지 않는다.
...
[연산자 오버로딩](operator-overloading.html)을 참고한다.
...
피연산자 `a`와 `b`가 정적으로 `Float`나 `Double`이거나 또는 이에 해당하는 nullable 타입일 때
(타입을 선언하거나 추정하거나 또는 [스마트 변환](typecasts.html#smart-casts)의 결과),
숫자나 범위에 대한 연산은 부동소수 연산에 대한 IEEE 754 표준을 따른다.

하지만 범용적인 유스 케이스를 지원하고 전체 순서를 제공하기 위해,
피연산자가 정적으로 부동소수점 타입이 아니면(예 `Any`, `Comparable<...>`, 타입 파라미터)
연산은 `Float`과 `Double`을 위한 `equals`와 `compareTo` 구현을 사용한다. 이는 표준과 다음이 다르다.
...
`Char` 타입으로 문자를 표현한다. 이 타입을 숫자로 바로 다룰 수 없다.
...
숫자와 마찬가지로, 문자도 nullable 레퍼런스가 필요하면 박싱된다. 박싱 연산을 하면 동일성은 유지되지 않는다.

## 불리언

`Boolean` 타입은 booleans을 표현하고 두 값이 존재한다: *true*{: .keyword }와 *false*{: .keyword }.

nullable 레퍼런스가 필요하면 불리얼도 박싱된다.

불리언에 대한 내장 연산은 다음이 있다
...
코틀리은 `Array` 클래스로 배열을 표현하며, 이 클래스는 `get`과 `set` 함수(연산자 오버로딩으로 `[]`로 바뀌는),
`size` 프로퍼티 그외 유용한 멤버 함수를 제공한다:
...
라이브러리 함수 `arrayOf()`를 사용해서 배열을 생성할 수 있다. 이 함수에는 항목 값을 전달하는데 `arrayOf(1, 2, 3)`는 [1, 2, 3] 배열을 생성한다.
`arrayOfNulls()` 라이브러리 함수를 사용하면 주어진 크기의 null로 채운 배열을 생성할 수 있다.

배열을 생성하는 다른 방법은 팩토리 함수를 사용하는 것이다. 팩토리 함수는 배열 크기와 해당 인덱스의 요소의 초기값을 리턴하는 함수를 인자로 받는다:
...
위에서 말했듯이, `[]` 연산자는 `get()`과 `set()` 멤버 함수의 호출을 의미한다.

노트: 자바와 달리 코틀린 배열은 무공변(invariant)이다. 이는 코틀린은 `Array<String>`를 `Array<Any>`에 할당할 수 없음을 의미하며, 이는 런타임 실패를 방지한다.
(하지만 `Array<out Any>`을 사용할 수 있다. [Type Projections](generics.html#type-projections)을 참고하자.)
 
코틀린은 또한 `ByteArray`, `ShortArray`, `IntArray` 등 박싱 오버헤드가 없는 기본 타입의 배열을 표현하기 위한
특수 클래스를 제공한다. 이 클래스는 Array 클래스와 상속 관계에 있지 않지만 같은 메서드와 프로퍼티가 있다. 각각에 대응한 팩토리 함수를 제공한다:
...
다른 하나는 원시 문자열로 newline과 임의 텍스트를 가질 수 있다.
escaped 문자열은 자바 문자열과 거의 똑같다:

``` kotlin
val s = "Hello, world!\n"
```

탈출 문자는 전형적인 방식인 백슬래시를 사용한다.
지원하는 탈출 문자 목록은 앞서 [문자](#characters)를 참고한다.

원시 문자열은 세 개의 따옴표로 구분하며(`"""`),
탈출 문자를 포함하지 않고 newline과 모든 다른 문자를 포함할 수 있다:
...
문자열은 템플릿 식을 포함할 수 있다. 예를 들어, 코드 조각을 계산하고 그 결과를 문자열에 연결할 수 있다.
...
원시 문자열과 escaped 문자열 안에 템플릿을 넣을 수 있다.
백슬래시 탈출 문자를 지원하지 않는 원시 문자열에서 `$` 문자를 표현하고 싶다면
다음 구문을 사용한다:
