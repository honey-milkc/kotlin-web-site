...
하지만 범용적인 유스 케이스를 지원하고 완전한 순서를 제공하기 위해,
...
주의: 자바와 달리 코틀린 배열은 무공변(invariant)이다. 이는 코틀린은 `Array<String>`를 `Array<Any>`에 할당할 수 없음을 의미하며, 런타임 실패를 방지한다.
...
다른 하나는 raw 문자열로 newline과 임의 텍스트를 가질 수 있다.
...
역슬래시 탈출 문자를 지원하지 않는 raw 문자열에서 `$` 문자를 표현하고 싶다면
다음 구문을 사용한다:
